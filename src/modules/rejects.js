...
const RejectAnalysis = {
    ...
    updateStatistics: function (rejects) {
        ...
        // Calculer par machine : quantité × prix unitaire moyen
        const machines = {};
        rejects.forEach(r => {
        ...
        // Calculer le coût total : somme de (quantité × prix moyen) par machine
        let totalCost = 0;
        const costByMachine = {};

        Object.keys(machines).forEach(machine => {
            const m = machines[machine];
            // Prix moyen de la machine (uniquement si des prix existent)
            const avgPrice = m.prices.length > 0
                ? m.prices.reduce((a, b) => a + b, 0) / m.prices.length
                : 0;
            // Coût = quantité totale × prix moyen
            const machineCost = m.qty * avgPrice;
            costByMachine[machine] = machineCost;
    ...
    displayMachineCostsTable: function (rejects) {
        ...
        // Calculer le coût par machine : quantité × prix moyen
        Object.keys(machines).forEach(machine => {
            const m = machines[machine];
            const avgPrice = m.prices.length > 0
                ? m.prices.reduce((a, b) => a + b, 0) / m.prices.length
