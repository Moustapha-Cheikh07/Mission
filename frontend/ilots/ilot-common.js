...
function initializeDateFilters() {
    const now = new Date();
    const startDate = new Date(now.getFullYear(), now.getMonth(), 1); // Premier jour du mois actuel
    const endDate = new Date(now.getFullYear(), now.getMonth() + 1, 0); // Dernier jour du mois actuel

    // Les inputs de date ne sont plus visibles, mais on les garde pour la logique de filtrage
